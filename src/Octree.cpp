...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	root.box.parameters[1] = root.box.parameters[1] + Vector3(eps, eps, eps); // !
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	// Subdivide current node's box into 8 children
	vector<Box> boxList;
	subDivideBox8(node.box, boxList);

	for (auto& childBox : boxList) {
		vector<int> childPoints;
		int count = getMeshPointsInBox(mesh, node.points, childBox, childPoints);
		if (count > 0) {
			TreeNode child;
			child.box = childBox;
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	...
	// Reject if ray misses node
	if (!node.box.intersect(ray, 0.0f, 10000.0f)) return false;
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	// Reject if boxes do not overlap
	...
	for (auto& child : node.children) {
		if (intersect(box, child, boxListRtn)) hit = true;
	}
	return hit;
}
...
void Octree::draw(TreeNode & node, int numLevels, int level) {
	...
	ofSetColor(levelColors[level % numColors]);
