...
void ParticleEmitter::spawn(float time) {
	...
	case ConeEmitter:
		...
		float maxTheta = glm::radians(coneAngle);
		float theta = ofRandom(0, maxTheta);
		ofVec3f local(
			sin(theta) * cos(phi),
			sin(theta) * sin(phi),
			cos(theta));
		ofQuaternion q;
		q.makeRotate(ofVec3f(0, 0, 1), coneDirection.getNormalized());
		particle.velocity = q * local * speed;
		particle.position.set(position);
		break;
	}
	...
	// cout << "Spawned particle   life=" << particle.lifespan << " vel=" << particle.velocity << endl;
